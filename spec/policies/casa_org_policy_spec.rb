  ...
  permissions :edit?, :update? do
    ...
    context "when admin does not belong to org" do
      it "does not permit admin" do
        expect(subject).to_not permit(casa_admin, different_organization)
      end
    end

    it "does not permit supervisor" do
      expect(subject).to_not permit(supervisor, organization)
    end

    it "does not permit volunteer" do
      expect(subject).to_not permit(volunteer, organization)
    end
  end
end
